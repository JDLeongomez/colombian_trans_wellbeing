...
avg.mod.summary <- function(avg_model, data) {
  ...
  extract_relevant_reference_levels <- function(avg_model, data) {
    ...
    relevant_factors <- factor_vars[sapply(factor_vars, function(var) any(grepl(paste0("^", var), model_terms)))]
    ...
    if (length(relevant_refs) > 0) {
      paste0("Reference levels: ", paste0(names(relevant_refs), " = ", relevant_refs, collapse = "; "))
    } else {
  ...
  summ_table <- summary(avg_model)$coefmat.full |>  
    ...
    kable(digits = 3,
          ...
          linesep = "",
          align = c("l", rep("c", 4)),
          caption = paste0("Coefficient estimates for the ", 
...
[^1]: To maximize the number of non-missing cases, we created separate datasets for each outcome variable. Since models generated by `dredge()` must not contain missing values to remain comparable, this approach ensures that each model includes the largest possible sample size. 
...
avg.mod.summary(avg_LS, data = dat_LS)
```
